...
if (registerForm) {
  registerForm.addEventListener('submit', async (e) => {
    ...
    if (!isValidEmail(email)) {
      showAuthError(errEl, 'Ingresa un correo electrónico válido.');
      return;
    }
    if (password !== confirm) {
    ...
    if (!registerForm.terms?.checked) {
      showAuthError(errEl, 'Acepta los términos de uso para continuar.');
      return;
    }
...
function validateRegisterInput({ name, email, password }) {
  if (!name || name.trim().length < 2) {
    const err = new Error('Ingresa tu nombre completo.');
